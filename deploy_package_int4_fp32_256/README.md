# pocket-tts 한국어 TTS — 배포 패키지 (deploy_package_int4_fp32_256)
...
- **Context max_len**: 256
- **Step Backbone**: INT4 (Weight-compressed, group=32) (~169MB)
- **Sampler (Flow + EOS)**: FP32 (~18MB)
- **Mimi Decoder**: FP32 (2048ctx) (~20MB)
- **전체 패키지 크기**: 약 310MB

```
deploy_package_int4_fp32_256/
├── README.md          패키지 안내 및 사양
├── MANUAL.md          상세 사용 가이드 및 화자 변경법
├── synth.py           음성 합성 진입점 (문장 분할·청킹 자동 지원)
├── models/            OpenVINO IR 모델
│   ├── step.xml/bin     Backbone Step (INT4 (Weight-compressed, group=32))
│   ├── sampler.xml/bin  Sampler Flow + EOS (FP32)
